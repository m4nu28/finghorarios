  ...
  return (
    <div className="card">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center gap-2 text-left">
        <label className="text-xs font-medium text-neutral-500 dark:text-neutral-400 flex-1 cursor-pointer">Horarios fijos</label>
        {fixedCourses.length > 0 && !open && (
          <span className="text-[11px] text-neutral-400 font-medium mr-1">{fixedCourses.length}</span>
        )}
        <svg className={`w-4 h-4 text-neutral-400 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      {open && (
        <div className="mt-4 animate-slide-down">
          <p className="text-xs text-neutral-400 dark:text-neutral-500 mb-4">Materias con horario asignado que el planificador debe respetar.</p>
          ...
          {fixedCourses.length > 0 && (
            <div className="space-y-1.5 mb-4">
              {fixedCourses.map((course, i) => (
                <div
                  key={i}
                  className={`rounded-xl px-3.5 py-2.5 transition-colors group ${
                    editingIdx === i
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleEdit(i)} className="text-neutral-300 hover:text-accent-600 transition-colors p-0.5">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                        </svg>
                      </button>
                      <button onClick={() => handleRemoveCourse(i)} className="text-neutral-300 hover:text-red-500 transition-colors p-0.5">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {course.blocks.map((b, bi) => (
                      <span key={bi} className="text-[10px] bg-neutral-100 dark:bg-neutral-800 text-neutral-500 dark:text-neutral-400 rounded-lg px-2 py-0.5 font-medium">
          ...
          <div className="bg-neutral-50 dark:bg-neutral-800/50 rounded-2xl p-4">
            <div className="text-[11px] font-medium text-neutral-400 mb-3">{editingIdx !== null ? 'Editando' : 'Nueva materia'}</div>
            ...
            {draftBlocks.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-3">
                {draftBlocks.map((b, i) => (
                  <span key={i} className="inline-flex items-center gap-1 text-[10px] bg-white dark:bg-neutral-700 border border-neutral-200 dark:border-neutral-600 rounded-lg px-2 py-0.5 font-medium text-neutral-600 dark:text-neutral-300">
                    {DAY_NAMES[b.day]?.slice(0, 3) || `D${b.day}`} {b.start}–{b.end}
                    <button onClick={() => handleRemoveDraft(i)} className="text-neutral-400 hover:text-red-500 ml-0.5">
